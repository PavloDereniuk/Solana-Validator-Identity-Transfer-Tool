...
export async function transferTower(
  ...
  if (r.code !== 0) {
    throw new Error(`read tower from ${from.host} failed: ${r.stderr || r.stdout}`);
  }
  ...
  if (b64.length === 0) {
    ...
    throw new Error(`refused: tower file at ${fromLedger}/tower-1_9-${pubkey}.bin on ${from.host} is empty`);
  }

  const w = await exec(to, cmdWriteTower(toLedger, pubkey), b64);
  if (w.code !== 0) {
    throw new Error(`write tower to ${to.host} failed: ${w.stderr || w.stdout}`);
  }
